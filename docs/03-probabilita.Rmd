...
subtitle: "Fondamenti di Probabilità"
...
Un __evento__ è un qualsiasi insieme di risultati di un esperimento probabilistico (ovvero, un sottoinsieme di $\Omega$ ).

Un __risultato__ di un esperimento probabilistico _verifica_ un evento se ne fa parte.

La __probabilità__ di un evento è la .orange[frequenza relativa] dei risultati che lo definiscono, valutata su **infinite ripetizioni** dell'esperimento probabilistico.

> Es.: 
- Esperimento: misura della pressione diastolica (DBP) di una persona presa a caso, in un determinato momento, nella popolazione.
- Spazio degli eventi $\Omega$: ogni possibile misura di DBP.
- Evento $A$ : DBP $\geq 95$ mmHg.
- Risultato (mio, ora): DBP $= 70$ mmHg. (Non fa parte dell'evento, quindi non lo verifica)
- Probabilità di $A$ : misurando la DBP, ora, a tutta la popolazione, la porzione di persone con DBP $\geq 95$ mmHg.
...
---
# .orange[Teorema della somma]: es.

Due medici, $A$ e $B$, diagnosticano tutti i pazienti afferenti a un centro per le malattie a trasmissione sessuale.
...
Si definisca come **famiglia ipertesa** quella in cui .orange[almeno uno] dei genitori è iperteso.

**Allora** (ipotizzando $A$ e $B$ indipendenti):

$$\begin{split} P(\{\mbox{famiglia ipertesa}\})& = P(A \cup B)\\&= P(A) + P(B\cap\bar{A})\\&= P(A) + P(B)P(\bar{A})\\& = P(A) +  P(B)[1-P(A)]\\& = 0.1 + (0.2)[1-0.1]\\& = 0.28\end{split}$$
...
Per me: [Probabilità](https://app.wooclap.com/events/STATMEDPROB/0)

Per voi: <img src="img/bit.ly_statmed-sondaggio-prob.png", width="300px" style="vertical-align: middle"/> [bit.ly/statmed-sondaggio-prob](https://bit.ly/statmed-sondaggio-prob)
...
- l'1\% della popolazione abbia SKT positivo

**Si calcoli** la .orange[probabilità di tubercolosi (TB) nella popolazione] $P(TB)$, ovvero, si sitmi la porzione di malati di tubercolosi nella popolazione (chiamata: la **prevalenza** della malattia)
...
**Allora**, per qualunque evento $B$, la probabilità di $B$, $P(B)$, può essere scritta come:

$$\begin{split}P(B)& =\sum_{i=1}^k P(B|A_i)P(A_i)\\&\\&=\sum_{A\in \mathscr{A}} P(B|A)P(A)\end{split}$$
Ovvero, la **somma pesata** delle probabilità condizionali di $B$ dato $A_i$, $P(B|A_i)$ per le corrispondenti probabilità degli $A_i$ al variare di $i$ tra $1$ e $k$.
...
> NOTA 2:
>
> Definiamo **Rischio assoluto** il rapporto tra i casi osservati e i casi osservabili (osservabili vuol, dire: che sono sottoposti al rischio con una probabilità positiva che si possa verificare l'evento di interesse), ovvero la probabilità che un certo evento accada sotto determinate condizioni in una data popolazione.
>
...
- la probabilità che un normopeso sia .orange[erroneamente classificato] come iperteso, sia pari a 0.23.
...
La formula "tradizionale" frequentista per VPP (analoga VPN)

$$VPP = \frac{VP}{VP + FP}$$

Dove $VP$ è il numero di **V**eri **P**ositivi (numero di volte che il test ha dato esito positivo correttamente) e $FP$ il numero di **F**alsi **P**ositivi (numero di volte che il test ha dato esito positivo erroneamente). Questo, dunque, ipotizzando di avere il dato certo del numero di veri e falsi positivi su tutta la nostra popolazione. 

Noi usiamo quella bayesiana, perché:

- esprime e mostra  bene il concetto di probabilità condizionata

- opera un cambio di prospettiva: dal controllo dell'incertezza su una _predizione_ sul futuro, rispetto a una descrizione statica definita di dati già osservati.

**SE** usati bene, in un certo senso, numericamente, sono equivalenti; per approfondire: https://rpubs.com/CorradoLanera/vpp-freq-bayes-ita

> NOTA: se usati male i risultati sono sia diversi che fuorvianti. All'esame suggerisco di non prendere questa _scorciatoia_. (per quanto SE usata bene, porti comunque al risultato numericamente corretto)
...
Per me: [Bayes](https://app.wooclap.com/events/STATMEDBAYES/0)

Per voi: <img src="img/bit.ly_statmed-sondaggio-bayes.png", width="300px" style="vertical-align: middle"/> [bit.ly/statmed-sondaggio-bayes](https://bit.ly/statmed-sondaggio-bayes)
...
- [Dimostrazione della Coerenza Bayesiana e Frequentista nella definizione del Valore Predittivo Positivo di un test](https://rpubs.com/CorradoLanera/vpp-freq-bayes-ita) di [**Corrado Lanera**](https://www.corradolanera.it/)
